...
- **Grouped views for Completed** — switch to "by month" or "by year" with a popover toggle. Two banner styles: full-width row banners between groups, or inline banners (CSS subgrid) where the next group's label fills the trailing slot of the previous row.
...
- **Editor modal** with `←/→` per-section navigation, status autofills (`→ Watching` fills started date, `→ Completed` fills progress to max and finished date), and inline "Mark Completed" nudges. Opens for guests too — form disabled with a "Sign in to track" CTA.
...
- Tokens last 1 year and live in `localStorage`. The GraphQL client only attaches `Authorization` when a token is present, so browse-only routes work for anonymous users.
- Visiting `/anime/list/*` as a guest silently redirects to `/anime/browse` — no login wall.
